...
pub fn run(yaml: &Value, environment: &String, skip_prompt: bool) {
    ...
    let dependencies = helpers::collect_dependencies(&yaml, &environment, &mut visited);
    let dependency_commands = helpers::collect_commands_for_dependencies(&yaml, &dependencies);
